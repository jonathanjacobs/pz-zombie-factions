...
| [CDDA Zombies](https://steamcommunity.com/sharedfiles/filedetails/?id=2749928925) | HW_Lover's Workshop | Global-ModData registry of zombie types with multiplayer sync, keyed on `getOnlineID()`. Read for [`spikes/SPIKE-006`](spikes/SPIKE-006-faction-identity-persistence.md) as an example of the recycled-handle defect that Special Zombies removed. |
...
| [Occult Zombies (B42)](https://steamcommunity.com/sharedfiles/filedetails/?id=3042358369) | Mystery's Workshop | Assigns zombie types entirely through `ZombiesZoneDefinition` and `HairOutfitDefinitions` with no runtime Lua, no events, and no state. The shipped precedent for carrying a durable per-zombie tag on the persistent outfit id; see [`spikes/SPIKE-006`](spikes/SPIKE-006-faction-identity-persistence.md). |
| [Random Zombies](https://steamcommunity.com/sharedfiles/filedetails/?id=2818577583) | belette's Workshop | Sandbox-configurable zombie-type population mix using a hash of `getOnlineID()` into a type distribution, applied in batches on `OnTick`. Confirmed for [`spikes/SPIKE-006`](spikes/SPIKE-006-faction-identity-persistence.md) as an example of assignment that is re-derived every session rather than persisted. |
...
| [Special Zombies Framework (B42)](https://steamcommunity.com/workshop/filedetails/?id=3783282009) | OO's Workshop | Persistent special-zombie registration/spawning shared framework; the closest precedent for our own faction-identity persistence and sync problem. Versions 1.15.3 and [1.16.0 fixed reupload](https://steamcommunity.com/sharedfiles/filedetails/?id=3794452652) were both read for [`spikes/SPIKE-006`](spikes/SPIKE-006-faction-identity-persistence.md); the difference between them is the evidence, because 1.16.0 withdraws the position-and-appearance recovery that 1.15.3 shipped. |
