...
5. Review `[ZombieFactions][PERF]` and `[ZombieFactions][SERVER_PERF]` summaries for errors, arm/collision counts, defender reactions, accepted damage, native-target clears, and distance rejections.
...
- For v0.0.32, begin with standing 1v1 and confirm repeated full bites with `impactExactTarget=0`, no `AttackState.triggerPlayerReaction` exception, and a visible target reaction when `hitReactionsArmed` increments. Then test two-client ownership, death, target loss, standing crowds, and finally mixed crawler crowds.
