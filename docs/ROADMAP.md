...
- [ ] Validate the v0.0.45 `NEUTRAL` retaliation behavior: that a Neutral zombie does not initiate, that it answers a validated attack, that hostility reaches the individual attacker and never its faction or mob, that an already-engaged victim keeps its fight while free neighbors answer, and that the authorization lapses cleanly. The discriminating case is a second attacker-faction zombie that never attacks and must be ignored throughout.
...
- [ ] Persist faction assignment across supported lifecycle transitions. [`SPIKE-006`](spikes/SPIKE-006-faction-identity-persistence.md) establishes from source that storing it in zombie `modData` cannot work, because the only zombie persistence in Build 42.20 is a fixed 21-byte population record with no extension point, and virtualization, chunk unload, and restart all read and write that same record. The persistent outfit id is the one per-zombie field in that record that makes a full round trip, it is reachable from Lua through the `ZombiesZoneDefinition` table, and the spike recommends carrying faction on it. A second route, the reanimated-player save path, does keep full `modData` per zombie but stops those zombies from ever virtualizing, so the spike recommends it only for a bounded number of individuals and records it as untested. This blocks [#19](https://github.com/jonathanjacobs/pz-zombie-factions/issues/19). Territory stays deferred: `dressInPersistentOutfit` assigns an outfit to an individual zombie directly, so no zone mapping is needed.
